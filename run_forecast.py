...
from services.multi_sector_farm import load_farm_for_analysis, parse_sectors_param


def _parse_args():
    parser = argparse.ArgumentParser(description="Run FarmBiddy forecast for the multi-sector farm.")
    parser.add_argument(
        "--farm-file",
        default=MULTI_SECTOR_FILE,
        help=f"Farm JSON in datasets/ (default: {MULTI_SECTOR_FILE})",
    )
    parser.add_argument(
        "--sectors",
        default="dairy,beef,lamb",
        help="Comma-separated sectors: dairy, beef, lamb (default: all three)",
    )
...
def main():
    args = _parse_args()
    sectors = parse_sectors_param(args.sectors)
    farm = load_farm_for_analysis(args.farm_file, sectors)

    context = ForecastContext(farm)

    print("Farm file:", args.farm_file)
    ...
    profit_margin = profit / milk_revenue if milk_revenue else 0
    ...
    print("Annual Revenue: €", round(milk_revenue, 2))
    ...
    profitability_dashboard = generate_profitability_dashboard(forecast_result, farm)
